import {Constant, InjectorService, CliFs, Command, CommandProvider, Inject, Type} from "@tsed/cli-core";
...
export class GenerateSwaggerCmd implements CommandProvider {
  ...
  private async loadPlatformModule(): Promise<{bootstrap(module: any, settings: any): Promise<PlatformAdapter & {injector: InjectorService, stop(): Promise<void>}>}> {
  ...
  private async generate($ctx: GenerateSwaggerCtx) {
    ...
    const confs = platform.injector.settings.get<SwaggerSettings[]>("swagger", []);
  ...
  private async generateFromSpec(spec: any, conf: SwaggerSettings, $ctx: GenerateSwaggerCtx) {
